...
\item[(2)] Prove the formulas for the standard errors in Theorem 13.8. You should
regard the $X_i$'s as fixed constants.
...
\item[(3)] Consider the \textbf{regression through the origin} model:
\[
Y_i = \beta X_i + \epsilon_i
\]
Find the least squares estimate for $\beta$. Find the standard error of the
...
\item[(6)] Get the passenger car mileage data from

\href{http://lib.stat.cmu.edu/DASL/Datafiles/carmpgdat.html}
{http://lib.stat.cmu.edu/DASL/Datafiles/carmpgdat.html}
\begin{enumerate}[(a)]
\item Fit a simple linear regression model to predict MPG (miles per
gallon) from HP (horsepower). Summarize your analysis including a
plot of the data with the fitted line.
\item Repeat the analysis but use $\log(\text{MPG})$ as the response. Compare
the analyses.
\end{enumerate}
...
\item[(7)] Get the passenger car mileage data from

\href{http://lib.stat.cmu.edu/DASL/Datafiles/carmpgdat.html}
{http://lib.stat.cmu.edu/DASL/Datafiles/carmpgdat.html}
\begin{enumerate}[(a)]
\item Fit a multiple linear regression model to predict MPG (miles per
gallon) from the other variables. Summarize your analysis.
\item Use Mallow $C_p$ to select a best sub-model. To search through the
...
\item Perform all possible regressions. Compare $C_p$ and BIC. Compare the
results.
\end{enumerate}
...
\item[(8)] Assume a linear regression model with Normal errors. Take $\sigma$ known.
Show that the model with highest AIC (equation (13.27)) is the model
with the lowest Mallows $C_p$ statistic.
...
\item[(11)] Get the Coronary Risk-Factor Study (CORIS) data from the book web
