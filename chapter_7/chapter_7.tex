...
\begin{enumerate}
\item[(2)] Let $X_1,\ldots,X_n \sim \Bern(p)$ and let
$Y_1, \ldots, Y_m \sim \Bern(q)$. Find
the plug-in estimator and estimated standard error for $p$. Find an approximate
90 percent confidence interval for $p$. Find the plug-in estimator
and estimated standard error for $p - q$. Find an approximate 90
...
\item[(6)]
Let $X_1, \ldots, X_n \sim F$ and let $\hat F$ be the empirical distribution
function. Let $a < b$ be fixed numbers and define $\theta = T(F) = F(b) - F(a)$.
Let $\hat \theta = T(\hat F_n) = \hat F_n(b) - \hat F_n(a)$.
...
\item[(7)] Data on the magnitudes of earthquakes near Fiji are available on the
...
\item[(8)] Get the data on eruption times and waiting times between eruptions of
...
\item[(9)] 100 people are given a standard antibiotic to treat an infection and
another 100 are given a new antibiotic. In the first group, 90 people
recover; in the second group, 85 people recover. Let $p_1$ be the probability
of recovery under the standard treatment and let $p_2$ be the probability of
recovery under the new treatment. We are interested in estimating
$\theta=p_1-p_2$· Provide an estimate, standard error, an 80 percent confidence
interval, and a 95 percent confidence interval for $\theta$.
...
\item In 1975, an experiment was conducted to see if cloud seeding produced
...
\begin{center}
\href{http://lib.stat.cmu.edu/DASL/Stories/CloudSeeding.html}
...
Estimate $\theta$. Estimate the standard error of the estimate and produce a
