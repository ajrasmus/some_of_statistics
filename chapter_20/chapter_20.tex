...
\item[(1)]
Let $X_1,\ldots,X_n\sim f$ and let $\hat f_n$ be the kernel density
estimator using the boxcar kernel:
\[
K(x) = \begin{cases}
...
\item Show that
\[
\E(\hat f_n(x)) = \frac{1}{h}\int_{x-(h/2)}^{x+(h/2)}f(y)dy
\]
and
\[
\V(\hat f_n(x)) = \frac{1}{nh^2}\left[ \int_{x-(h/2)}^{x+(h/2)} f(y)dy -
\left(\int_{x-(h/2)}^{x+(h/2)}f(y)dy\right)^2\right]
\]

\item Show that if $h\to 0$ and $nh\to\infty$ as $n\to\infty$,
then $\hat f_n(x)\xrightarrow{P} f(x)$.
\end{enumerate}
...
\item[(2)] Get the data on fragments of glass collected in forensic work from the
book website. Estimate the density of the first variable (refractive index)
using a histogram and using a kernel density estimator. Use cross-validation
to choose the amount of smoothing. Experiment with different
...
\item[(3)] Consider the data from question 2. Let $Y$ be refractive index and let
$x$ be aluminum content (the fourth variable). Perform a nonparametric
regression to fit the model $Y = f(x) + \epsilon$. Use cross-validation to estimate
