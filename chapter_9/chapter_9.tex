...
\item[(5)] Let $X_1,\ldots,X_n\sim \Poiss(\lambda)$. Find the method of moments estimator,
...
\item[(8)] Find the Fisher information matrix for Example 9.29.
...
\item[(10)] Let $X_1,\ldots, X_n \sim \Unif(0,\theta)$. The $\MLE$ is
$\hat\theta = X_{(n)}=\max\{X_1,\ldots,X_n\}$.
Generate a dataset of size $50$ with $\theta=1$.
\begin{enumerate}[(a)]
\item Find the distribution of $\hat \theta$ analytically.
Compare the true distribution
of $\hat \theta$ to the histograms from the parametric and nonparametric
bootstraps.
\item This is a case where the nonparametric bootstrap does very poorly.
Show that for the parametric bootstrap $\P(\hat \theta^*=\hat \theta)=0$,
but for the
nonparametric bootstrap $\P(\hat \theta^*=\hat\theta)\approx .632$.
Hint: show that, $\P(\hat \theta^*=\hat \theta)=1 - (1-(1/n))^n$
then take the limit as $n$ gets large. What is the implication of this?
\end{enumerate}
